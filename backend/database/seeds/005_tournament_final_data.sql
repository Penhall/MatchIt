-- database/seeds/005_tournament_final_data.sql
-- Dados de exemplo para torneios - versão final com transações separadas

-- TRANSAÇÃO 1: Adicionar valores do enum necessários
BEGIN;

-- Verificar estrutura atual
DO $$
BEGIN
    RAISE NOTICE 'Verificando e adicionando valores do enum tournament_category_enum...';
END $$;

-- Adicionar valores do enum que podem estar faltando
DO $$
BEGIN
    ...
    RAISE NOTICE 'Enum values adicionados com sucesso - commit necessário antes de usar';
END $$;

COMMIT;

-- TRANSAÇÃO 2: Inserir dados (após commit dos enum values)
BEGIN;

DO $$
BEGIN
    RAISE NOTICE 'Iniciando inserção de dados...';
    RAISE NOTICE 'Valores do enum disponíveis após commit:';
END $$;

-- Mostrar valores atuais do enum após commit
DO $$
DECLARE
    enum_value TEXT;
BEGIN
...
-- Inserir dados usando as categorias disponíveis
