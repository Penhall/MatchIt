-- database/seeds/005_tournament_compatible_data.sql
-- Dados de exemplo para torneios - compatível com enum atual

-- Verificar estrutura atual
DO $$
BEGIN
    RAISE NOTICE 'Inserindo dados na estrutura atual da tabela tournament_images...';
    RAISE NOTICE 'Verificando enum values disponíveis...';
END $$;

-- Verificar e potencialmente adicionar valores do enum que podem estar faltando
DO $$
BEGIN
    -- Tentar adicionar 'estilos' se não existir
    BEGIN
    ...
    EXCEPTION
        ...
        WHEN others THEN
            RAISE NOTICE 'Não foi possível adicionar valor estilos: %', SQLERRM;
    END;
    
    -- Tentar adicionar 'ambientes' se não existir  
    BEGIN
    ...
    EXCEPTION
        WHEN duplicate_object THEN 
            NULL; -- Valor já existe
        WHEN others THEN
            RAISE NOTICE 'Não foi possível adicionar valor ambientes: %', SQLERRM;
    END;
END $$;
...
-- Inserir dados usando APENAS categorias que certamente existem no enum original
...
-- Categoria: acessorios (16 imagens - expandindo para ter mais opções)
...
('acessorios', 'https://via.placeholder.com/400x300/95A5A6/FFFFFF?text=Lenco+Seda', 'Lenço Seda', 13, true, ARRAY['lenço', 'seda', 'elegante'], true, 'Lenço de seda sofisticado'),
('acessorios', 'https://via.placeholder.com/400x300/E91E63/FFFFFF?text=Brinco+Moderno', 'Brinco Moderno', 14, true, ARRAY['brinco', 'moderno', 'estiloso'], true, 'Brinco moderno elegante'),
('acessorios', 'https://via.placeholder.com/400x300/009688/FFFFFF?text=Colar+Delicado', 'Colar Delicado', 15, true, ARRAY['colar', 'delicado', 'fino'], true, 'Colar delicado'),
('acessorios', 'https://via.placeholder.com/400x300/FF5722/FFFFFF?text=Anel+Design', 'Anel Design', 16, true, ARRAY['anel', 'design', 'exclusivo'], true, 'Anel de design exclusivo'),

-- Categoria: calcados (16 imagens - expandindo para ter mais opções)
...
('calcados', 'https://via.placeholder.com/400x300/009688/FFFFFF?text=Sapato+Social', 'Sapato Social', 15, true, ARRAY['sapato', 'social', 'elegante'], true, 'Sapato social elegante'),
('calcados', 'https://via.placeholder.com/400x300/FF5722/FFFFFF?text=Bota+Chelsea', 'Bota Chelsea', 16, true, ARRAY['bota', 'chelsea', 'moderna'], true, 'Bota Chelsea moderna'),

-- Categoria: texturas (16 imagens - expandindo para ter mais opções)
...
('texturas', 'https://via.placeholder.com/400x300/4682B4/FFFFFF?text=Denim+Classic', 'Denim Clássico', 11, true, ARRAY['denim', 'clássico', 'resistente'], true, 'Textura de denim clássico'),
('texturas', 'https://via.placeholder.com/400x300/228B22/FFFFFF?text=Algodao+Soft', 'Algodão Macio', 12, true, ARRAY['algodão', 'macio', 'natural'], true, 'Textura de algodão macio'),
('texturas', 'https://via.placeholder.com/400x300/800080/FFFFFF?text=Lycra+Stretch', 'Lycra Elástica', 13, true, ARRAY['lycra', 'elástica', 'confortável'], true, 'Textura de lycra elástica'),
('texturas', 'https://via.placeholder.com/400x300/FF6347/FFFFFF?text=Tweed+Elegante', 'Tweed Elegante', 14, true, ARRAY['tweed', 'elegante', 'sofisticado'], true, 'Textura de tweed elegante'),
('texturas', 'https://via.placeholder.com/400x300/32CD32/FFFFFF?text=Canvas+Resistente', 'Canvas Resistente', 15, true, ARRAY['canvas', 'resistente', 'durável'], true, 'Textura de canvas resistente'),
('texturas', 'https://via.placeholder.com/400x300/DC143C/FFFFFF?text=Malha+Comfort', 'Malha Confortável', 16, true, ARRAY['malha', 'confortável', 'flexível'], true, 'Textura de malha confortável');

-- Tentar inserir dados para categorias que podem existir
DO $$
BEGIN
    -- Tentar inserir dados para 'estilos' se existir
    BEGIN
        INSERT INTO tournament_images (category, image_url, image_name, display_order, active, tags, approved, alt_text) VALUES
        ...
        RAISE NOTICE 'Categoria estilos inserida com sucesso';
    EXCEPTION
        WHEN others THEN
            RAISE NOTICE 'Não foi possível inserir dados para estilos: %', SQLERRM;
    END;
    
    -- Tentar inserir dados para 'ambientes' se existir
    BEGIN
        INSERT INTO tournament_images (category, image_url, image_name, display_order, active, tags, approved, alt_text) VALUES
        ...
        ('ambientes', 'https://via.placeholder.com/400x300/FF9800/FFFFFF?text=Home+Office', 'Home Office', 9, true, ARRAY['home', 'office', 'produtivo'], true, 'Ambiente home office'),
        ('ambientes', 'https://via.placeholder.com/400x300/3F51B5/FFFFFF?text=Quarto+Casal', 'Quarto Casal', 10, true, ARRAY['quarto', 'casal', 'íntimo'], true, 'Ambiente quarto de casal'),
        ('ambientes', 'https://via.placeholder.com/400x300/795548/FFFFFF?text=Sala+TV', 'Sala de TV', 11, true, ARRAY['sala', 'tv', 'entretenimento'], true, 'Ambiente sala de TV'),
        ('ambientes', 'https://via.placeholder.com/400x300/607D8B/FFFFFF?text=Lavanderia', 'Lavanderia', 12, true, ARRAY['lavanderia', 'funcional', 'prática'], true, 'Ambiente lavanderia'),
        ('ambientes', 'https://via.placeholder.com/400x300/9C27B0/FFFFFF?text=Closet+Luxo', 'Closet Luxo', 13, true, ARRAY['closet', 'luxo', 'organizado'], true, 'Ambiente closet de luxo'),
        ('ambientes', 'https://via.placeholder.com/400x300/4CAF50/FFFFFF?text=Area+Gourmet', 'Área Gourmet', 14, true, ARRAY['área', 'gourmet', 'social'], true, 'Ambiente área gourmet'),
        ('ambientes', 'https://via.placeholder.com/400x300/FF5722/FFFFFF?text=Biblioteca', 'Biblioteca', 15, true, ARRAY['biblioteca', 'estudo', 'conhecimento'], true, 'Ambiente biblioteca'),
        ('ambientes', 'https://via.placeholder.com/400x300/009688/FFFFFF?text=Atelie+Arte', 'Ateliê de Arte', 16, true, ARRAY['ateliê', 'arte', 'criativo'], true, 'Ambiente ateliê de arte');
        
        RAISE NOTICE 'Categoria ambientes inserida com sucesso';
    EXCEPTION
        WHEN others THEN
            RAISE NOTICE 'Não foi possível inserir dados para ambientes: %', SQLERRM;
    END;
END $$;
