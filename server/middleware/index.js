...
if (config.features.enableRecommendations) {
  try {
    const { default: recRoutes } = await import('../routes/recommendations.js');
...
router.use('/', profileRoutes); // Para manter compatibilidade com /api/profile
...
if (config.nodeEnv === 'development') {
  ...
  console.log('  ✅ /api/profile, /api/style-choices');
