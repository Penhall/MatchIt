#!/bin/bash
# scripts/quick-build-fix.sh - Correção rápida para erro de build
...
echo -e "${BLUE}🔧 Correção Rápida - Build Error${NC}"

# Backup
cp screens/LoginScreen.tsx screens/LoginScreen.tsx.backup 2>/dev/null || true

# Substituir LoginScreen.tsx com versão que funciona
cat > screens/LoginScreen.tsx << 'EOF'
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useTranslation } from 'react-i18next';
// IMPORTS CORRIGIDOS - usando paths relativos que funcionam
...
echo -e "${YELLOW}📋 Agora teste: npm run build${NC}"